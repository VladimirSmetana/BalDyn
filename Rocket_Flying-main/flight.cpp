...
void flight ::flight_calculations(double (&kalph)[3], double (&kpeng)[2])
{
    ...
    while (sec->tY>0 && sec->V>0)
    {
        ...
        pit.anY[0] = fir->anY;
        pit.anY[1] = sec->anY;
        pit.peng[0] = fir->Peng_t;
        pit.peng[1] = sec->Peng_t;
        pit.h = h;
        pit.Ott = Ott;
        pit.mpn = mpn;
        pit.T_fuel = T_fuel[0];
        ...
        //qDebug()<<sec->tY;
        xn.push_back(pit.time);
        yu_1.push_back(pit.HSP_1);
        yu_2.push_back(pit.HSP_2);
        center_1.push_back(fir->gl_c);
        center_2.push_back(sec->gl_c);
        v_1.push_back(pit.vel[0]);
        v_2.push_back(pit.vel[1]);
        sinn.push_back(fir->Ssumm);
        jinn.push_back(fir->Isumm);
        jinn2.push_back(sec->Isumm);
        CM.push_back(fir->Ssumm/pit.mass[0]);
        mass_1.push_back(fir->m_t);
        mass_2.push_back(sec->m_t);
        Long_1.push_back(pit.Len[0]);
        Long_2.push_back(pit.Len[1]);
        w.push_back(pit.Wind2);
        H1.push_back(pit.H[0]);
        H2.push_back(pit.H[1]);
        angle.push_back(pit.pitch_angle*57.3);
        b1.push_back(pit.HSP_1*pit.Smid*pit.CX_1);
        b2.push_back(pit.HSP_2*pit.Smid*pit.CX_2);
        lin.push_back(Ix);
        ALI_1.push_back(pit.alpha[0]);
        ALI_2.push_back(pit.alpha[1]);
        //ALI.push_back(fir->Peng_t/ (fir->m_t*Atm_1.get_AOG()));
        res.push_back(sec->Peng_t/(sec->m_t*pit.AOG[1]));
        TET_1.push_back(pit.anY[0]*57.3);
        TET_2.push_back(pit.anY[1]*57.3);
        pi.push_back(pit.pitch_angle*57.3);
        P1.push_back(fir->Peng_t);
        P2.push_back(sec->Peng_t);
        f1.push_back(fir->focus);
        Lon.push_back(fir->Peng_t/(fir->m_t*pit.AOG[0]));
        Lonre.push_back(sec->Peng_t/(sec->m_t*pit.AOG[1]));
        pc2.push_back(sec->Peng_control);
        cy2.push_back(pit.CY_2);
